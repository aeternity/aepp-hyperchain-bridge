// copied this because builds were failing
...
const includesRegExp = new RegExp(
  includeRegExp.source,
  `${includeRegExp.flags}g`
);
  ...
  await Promise.all(
    (sourceCode.match(includesRegExp) ?? [])
      .map((include) => {
        ...
        if (m?.length !== 2) throw new Error("Unexpected match length");
        return m[1];
      })
 ...
 */
export default async function getFileSystem(
  path: string
): Promise<Record<string, string>> {
